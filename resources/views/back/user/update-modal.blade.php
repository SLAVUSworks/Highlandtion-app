        ...
        <div class="p-4">
            <form action="{{ url('back/users/'.$item->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nickname" id="nickname" class="w-full p-2 border rounded-lg @error('nickname') border-red-500 @enderror" value="{{ old('nickname', $item->nickname) }}">
                    @error('nickname')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="full_name" id="full_name" class="w-full p-2 border rounded-lg @error('full_name') border-red-500 @enderror" value="{{ old('full_name', $item->full_name) }}">
                    @error('full_name')
                    <div class="text-red-500 mt-2 text-sm">
                        {{ $message }}
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" id="email" class="w-full p-2 border rounded-lg @error('email') border-red-500 @enderror" value="{{ old('email', $item->email) }}">
                    @error('email')
                    <div class="text-red-500 mt-2 text-sm">
                        {{ $message }}
                    </div>
                ...
                <div class="flex justify-end space-x-2">
                    <button type="button" class="bg-gray-500 text-white px-4 py-2 rounded-lg close-modal" data-bs-target="#modalUpdate{{ $item->id }}">Batal</button>
                    <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded-lg">Ya, Edit!</button>
                </div>
            </form>
        </div>
    </div>

    ...
    <div class="mt-4 rounded-lg bg-white p-5 shadow-sm border border-gray-100">
        <form action="{{ route('back.articles.update', $article) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="title" name="title" class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300" value="{{ old('title', $article->title) }}" required>
            </div>
            <div class="mb-4">
                <label for="category" class="block text-gray-700 font-bold mb-2">Kategori</label>
                <input type="text" id="category" name="category" class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300" value="{{ old('category', $article->category) }}" required>
            </div>
            <div class="mb-4">
                <label for="desc" class="block text-gray-700 font-bold mb-2">Deskripsi</label>
                <textarea id="desc" name="desc" rows="5" class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300" required>{{ old('desc', $article->desc) }}</textarea>
            </div>
            <div class="mb-4">
                <label for="img" class="block text-gray-700 font-bold mb-2">Gambar</label>
                <input type="file" id="img" name="img" class="w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-sky-50 file:text-sky-700 hover:file:bg-sky-100 border border-gray-300 rounded-lg cursor-pointer">
                @if ($article->img)
            ...
            <div class="mb-4">
                ...
                <input type="date" id="publish_date" name="publish_date" class="w-full px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300" value="{{ old('publish_date', $article->publish_date) }}" required>
            </div>

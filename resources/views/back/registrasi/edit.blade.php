...
<div class="container mx-auto">
    <div class="mb-6 rounded-lg bg-white/60 backdrop-blur px-6 py-4 shadow-sm border border-gray-200">
        <h1 class="flex items-center gap-3 text-3xl font-semibold text-gray-800">
            ...
            Verifikasi Registrasi Peserta
        </h1>
        <p class="mt-1 text-sm text-gray-500">
            Review data registrasi peserta
        </p>
    </div>
    <div class="mt-4 rounded-lg bg-white p-5 shadow-sm border border-gray-100">
        <form id="registrasiForm" action="{{ route('back.registrasis.update', $registrasi->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <img src="{{ asset('storage/' . $registrasi->bukti_transfer) }}" alt="Bukti Pembayaran"
                    class="w-14 cursor-pointer" onclick="openModal(this)">
            </div>
            ...
            <div id="imageModal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center hidden"
                ...
                <div class="relative" id="modalContent" onclick="event.stopPropagation()">
                    <img id="modalImage" src="" alt="Modal View"
                        class="max-w-full max-h-screen rounded-lg transform transition-transform duration-300">
                    <button class="absolute top-2 right-2 text-black text-4xl font-bold cursor-pointer text-black"
                        onclick="closeModal()">×</button>
                    <div class="absolute bottom-2 left-2 flex space-x-2">
                        <button class="bg-white text-black px-2 py-1 rounded-lg" onclick="zoomIn()">+</button>
                        <button class="bg-white text-black px-2 py-1 rounded-lg" onclick="zoomOut()">-</button>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="ruangan_id" name="ruangan_id" required
                    ...
                    <option value="{{ $ruangan->id }}" @if($ruangan->kuota_now == $ruangan->kuota) disabled @endif>
                        {{ $ruangan->nama_ruangan }} - @if($ruangan->kuota_now ==
                        NULL)0/{{ $ruangan->kuota }}@else{{ $ruangan->kuota_now }}/{{ $ruangan->kuota }}@endif
                    </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <label for="confirmRuangan" class="text-gray-700 font-bold">Saya telah memastikan pembayaran tersebut
                    valid.</label>
            </div>
            @endif
        ...
        @if ($registrasi->status === 'rejected')
        <button type="button" id="restoreButton" class="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-600"
            data-url="{{ route('back.registrasis.restore', $registrasi->id) }}">
            Pulihkan
        </button>
        <form action="{{ route('back.registrasis.destroy', $registrasi->id) }}" method="POST"
        ...
        @else
        <button type="button" id="confirmButton" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600"
            disabled>
